...
def calc_distance_opt(cost_matrix_opt, path_tour):
    distance = 0
    for k in range(0, len(path_tour[0]) - 1):
        m = k + 1
        distance = distance + cost_matrix_opt[path_tour[0][k] - 1, path_tour[0][m] - 1]
    return distance


def mutation(best_solution, coord, Limiter):
    # Prepare date for GA
    ...
    for i in range(ITERATION):
        ...
        for k in range(int((len(population) - 2) / 2)):
            ...
            if random.random() < MUTATION_RATE:
                point1 = random.randint(0, lenGA_coord - 1)
                point2 = random.randint(0, lenGA_coord - 1)
        ...
        ga_cost = sorted(population)[0][0]  # print('Mutation Iteration ' + str(i + 1) + ' -> ' + str(ga_cost))
    best_solution_ga = []
...
def two_opt(distance_matrix, path, best_cost, Limiter):
    ...
    distance = path_list[1] * 2
    iteration = 0
    while (count < recursive_seeding):
        # print('Iteration = ', iteration, 'Distance = ', round(path_list[1], 2))
        best_route = copy.deepcopy(path_list)
        seed = copy.deepcopy(path_list)
        for i in range(0, len(path_list[0]) - 2):
            for j in range(i + 1, len(path_list[0]) - 1):
                best_route[0][i:j + 1] = list(reversed(best_route[0][i:j + 1]))
                best_route[0][-1] = best_route[0][0]
                best_route[1] = calc_distance_opt(distance_matrix, best_route)
                if (path_list[1] > best_route[1]):
                    path_list = copy.deepcopy(best_route)
                best_route = copy.deepcopy(seed)
        count = count + 1
        iteration = iteration + 1
        if (distance > path_list[1] and recursive_seeding < 0):
            distance = path_list[1]
            count = -2
            recursive_seeding = -1
        elif (path_list[1] >= distance and recursive_seeding < 0):
            count = -1
            recursive_seeding = -2
        if Limiter:
    ...
    def solve(self, new_graph: Graph, new_coord, graph: Graph, coord, new_points):
        ...
        for gen in range(self.generations):
            ...
            if ant_cost == former_best_cost:
                ga_cost, best_solution_ga = mutation(best_solution, copy.deepcopy(coord), False)
                opt_cost, best_solution_opt = two_opt(graph.matrix, best_solution_ga, ga_cost, False)
                break
            else:
                ga_cost, best_solution_ga = mutation(best_solution, copy.deepcopy(coord), True)
                opt_cost, best_solution_opt = two_opt(graph.matrix, best_solution_ga, ga_cost, True)
        ...
        if len(new_coord) > len(coord):
            ...
            ga_cost, best_solution_ga = mutation(best_solution, new_coord, False)
            opt_cost, best_solution_opt = two_opt(new_graph.matrix, best_solution_ga, ga_cost, False)
        #endregion
        print("\n============HACO==============")
        print("ACO Generation: " + str(gen + 1))
        print('ACO cost: ' + str(ant_cost) + '\nACO solution: ' + str(best_solution))
        print('Mutation cost: ' + str(ga_cost) + '\nGA solution: ' + str(best_solution_ga))
        print('2-opt cost: ' + str(opt_cost) + '\n2-opt solution: ' + str(best_solution_opt))

        return best_solution_opt, opt_cost, new_coord


# ga_cost, best_solution_ga = mutation(best_solution, coord)
# opt_cost, best_solution_opt = two_opt(graph.matrix, ant_cost, best_solution)
class _Ant(object):
    def __init__(self, haco: HACO, graph: Graph):
        self.colony = haco
        self.graph = graph
        self.total_cost = 0.0
